...
export default function ProductCard({ order, onDone }: ReceiptProps) {
  const [isCancceled, setIsCancelled] = useState(order.status === 'cancelled');
    ...
    <div className={`relative bg-white text-black rounded-2xl w-full max-w-sm overflow-hidden shadow-[0_16px_40px_rgb(0,0,0,0.10),0_4px_12px_rgb(0,0,0,0.06)] ${order.status ? 'bg-blue-500/15' : 'border-black'}`}>
      ...
      <div className="px-4 pt-4 pb-2">
        ...
        <div className="px-4 pb-4">
          <button
            className="w-full text-red-400 bg-white hover:text-white hover:bg-red-200 border-2 border-red-300 transition-all rounded-xl text-sm font-bold py-3 uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed"
            onClick={handleClick}
            disabled={isCancceled}
          >
            {isCancceled ? "Cancelled" : "Cancel Order"}
          </button>
        </div>
      </div>
    </div>
