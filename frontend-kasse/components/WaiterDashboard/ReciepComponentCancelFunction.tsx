...
export default function ProductCard({ order, onDone }: ReceiptProps) {
  ...
  return (
    <div className={`relative bg-white text-black p-6 shadow-md border-t-4 border-black font-mono w-full max-w-sm mb-6 ${order.status ? 'bg-blue-500/15' : 'border-black'}`}>
      ...
      <button
        className="w-full mt-2 text-red-400 bg-white hover:text-white hover:bg-red-200 border-2 border-red-300 transition-all rounded-md text-lg font-bold py-2 disabled:opacity-40 disabled:cursor-not-allowed"
        onClick={handleClick}
