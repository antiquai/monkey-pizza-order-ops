...
def apply_inventory_deduction(cur, required_totals, order_db_id: str):
    for ingredient_code, required_qty in required_totals.items():
        ...
        """, (
            ...
            "pizza_order",
            str(order_db_id),
            "preorder deduction on shift activation",
        ))
...
print("--- PREORDER WORKER READY ---", flush=True)

while True:
    task = r.blpop("preorder_deduct", timeout=0)
    if not task:
        continue
    ...
    try:
        ...
        print(f"Processing preorder deduction for order {db_order_id}", flush=True)

        conn = db_connect()
        cur = conn.cursor()

        try:
            required_totals = build_required_ingredients(cur, items)
            ...
            if missing:
                ...
                print(f"Preorder inventory blocked for order {db_order_id}: {missing}", flush=True)
            else:
                apply_inventory_deduction(cur, required_totals, db_order_id)
                conn.commit()
                set_order_inventory_status(db_order_id, "deducted", note="preorder inventory deducted on shift start")
                print(f"Preorder inventory deducted for order {db_order_id}", flush=True)

        except Exception as inventory_error:
            conn.rollback()
            set_order_inventory_status(db_order_id, "error", note=str(inventory_error))
            print(f"Preorder inventory error for order {db_order_id}: {inventory_error}", flush=True)
        ...
        print(f"Preorder worker error: {e}", flush=True)